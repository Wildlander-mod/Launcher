...
if (
  !fs.existsSync(homedir, (err) => {
    throw err;
  })
) {
  fs.mkdir(homedir, (err) => {
    throw err;
  });
}
// Check if ENB directory exists, if not, create one
const defaultENBPath = path.join(homedir, "/ENB Profiles/");
...
if (!fs.existsSync(path.join(homedir, "/launcher.json"))) {
  ...
  fs.writeFileSync(
    path.join(homedir, "launcher.json"),
...
ipcMain.on("launch-game", async (_event, args) => {
  const currentConfig = JSON.parse(
    fs.readFileSync(path.join(homedir, "launcher.json"))
  );
  ...
  const moPath = path.join(
    ...
    '\\ModOrganizer.exe -p "UltSky 4.0.7 (' + args + ' Preset)" SKSE'
  );
  childProcess.exec(moPath);
  const isSkyrimRunning = setInterval(checkProcess, 1000);

  function checkProcess() {
    // TODO this should be defined properly
    ...
    isRunning("ModOrganizer.exe", (status) => {
      if (!status) {
        ...
        fs.readdir(ENBPath, (err, files) => {
          ...
          files.forEach((file) => {
            fs.unlink(
              path.join(currentConfig.Options.GameDirectory, file),
              (err) => {
                console.log(err);
              }
            );
